              ...
              <Card className="border-white/5 bg-white/[0.02] backdrop-blur-md rounded-[2rem] overflow-hidden">
                ...
                <CardContent className="p-8 space-y-4">
                  {[
                    { 
                      name: "GitHub", 
                      handle: (user as any)?.provider === 'github' ? "Connected" : "Not connected", 
                      icon: Github, 
                      color: "bg-white text-black", 
                      status: (user as any)?.provider === 'github' ? 'Connected' : 'Disconnected' 
                    },
                    { 
                      name: "Cloudflare", 
                      handle: "Coming Soon", 
                    ...
                    <div key={i} className="flex items-center justify-between p-5 rounded-2xl border border-white/5 bg-white/[0.03] hover:bg-white/[0.06] transition-all group">
                      ...
                      <Button 
                        ...
                        disabled={item.status === 'Connected' || item.name !== 'GitHub'}
                        onClick={() => {
                          if (item.name === 'GitHub') {
                             // This will be handled by the Profile page link logic normally
                             // but we can add a simple toast or link here
                             toast.info("Please link your account in the Profile tab");
                          }
                        }}
                      >
                        {item.status === 'Connected' ? "Account Linked" : item.name === 'GitHub' ? "Connect" : "Unavailable"}
                      </Button>
                    </div>
                  ))}
                </CardContent>
