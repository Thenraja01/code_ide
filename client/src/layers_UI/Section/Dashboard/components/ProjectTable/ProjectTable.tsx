...
import { useProjectsQuery, useCreateProjectMutation, useDeleteProjectMutation, useToggleStarMutation } from "@/hooks/useProject.hooks";
...
export default function ProjectTable({ view }: ProjectTableProps) {
    ...
    const { mutate: toggleStar } = useToggleStarMutation();
    ...
    return (
        <section className="fade-in-up delay-300 flex flex-col gap-4">
            {/* Search + Filter Bar */}
            <Card className="p-2 md:p-3 rounded-2xl md:rounded-[1.5rem] border-white/5 bg-white/[0.02] backdrop-blur-sm">
                <CardContent className="flex flex-col lg:flex-row gap-3 p-0">
                    <div className="relative flex-1">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground/50" />
                        <Input
                            placeholder="Search your workspaces..."
                            className="pl-11 h-12 bg-white/[0.03] border-white/5 rounded-xl md:rounded-2xl"
                            value={search}
                    ...
                    <div className="flex items-center gap-2 overflow-x-auto pb-1 lg:pb-0 scrollbar-none">
                        {(["all", "starred", "recent"] as Filter[]).map((f) => (
                            <Button
                                ...
                                size="sm"
                                onClick={() => setFilter(f)}
                                className={`capitalize h-10 px-6 rounded-xl md:rounded-2xl font-bold transition-all ${filter === f ? 'bg-indigo-600 hover:bg-indigo-700 shadow-lg shadow-indigo-600/20' : 'bg-white/[0.03] hover:bg-white/[0.08] text-white/50'}`}
                            >
                                {f}
                            </Button>
                        ))}
            ...
            {/* Table */}
            <Card className="rounded-[2rem] overflow-hidden border-white/5 bg-white/[0.02] backdrop-blur-md shadow-2xl">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-8 pt-8 pb-4 gap-4">
                    <div className="space-y-1">
                        <h2 className="text-xl font-black text-white">Your Workspaces</h2>
                        <p className="text-xs text-white/30 uppercase tracking-[0.2em] font-bold">Active project environments</p>
                    </div>
                    <Button onClick={() => setIsDialogOpen(true)} className="w-full sm:w-auto bg-indigo-600 hover:bg-indigo-700 text-white font-black h-12 px-8 rounded-2xl shadow-lg shadow-indigo-600/20 group transition-all active:scale-95">
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-2">
                                                ...
                                                >
                                                    <Star
                                                        className={`h-4 w-4 transition-all ${project.isStarred ? 'fill-amber-500 text-amber-500' : 'text-muted-foreground group-hover/star:text-amber-500/50'}`}
                                                    />
                                                </button>
